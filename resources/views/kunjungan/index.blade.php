...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
                    ...
                    <td class="px-5 py-3.5">
                        <div class="flex items-center gap-1 justify-end">
                            <a href="{{ route('kunjungan.show', $k) }}"
                               class="p-1.5 text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded transition" title="Detail">
    ...
    <div class="px-5 py-3 border-t border-slate-100 flex items-center justify-between">
        <p class="text-xs text-slate-500">
            Menampilkan {{ $kunjungans->firstItem() }}–{{ $kunjungans->lastItem() }} dari {{ $kunjungans->total() }}
        </p>
        {{ $kunjungans->withQueryString()->links('vendor.pagination.tailwind') }}
    </div>
    @endif
</div>
